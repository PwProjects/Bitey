...
local DECORATIVES = {"green-asterisk-mini", "green-bush-mini", "green-carpet-grass", "green-hairy-grass", "green-small-grass"}
...
local function clear_nest_area(surface, position, radius, types)
    ...
    for _, ent in ipairs(entities) do
        if post_util.distance(ent.position, position) <= r then
            ent.destroy()
        end
    end
    ...
    for x = position.x - r, position.x + r do
        for y = position.y - r, position.y + r do
            if post_util.distance({x = x, y = y}, position) <= r then
                tiles[#tiles+1] = {
                    name = "dirt-5",
                    position = {x, y}
                }
...
function pet_nest.decorate(surface, position)
	...
	clear_nest_area(surface, position, 10, {"tree", "simple-entity"})
	...
	surface.create_entity {
		...
		position = {position.x + offset.x -1, position.y + offset.y - 1},
		force = "neutral"
	}

	surface.create_entity {
		name = "big-biter-corpse",
		position = {position.x + offset.x + 2, position.y + offset.y - 3},
		force = "neutral"
	}
	...
	surface.create_entity {
		name = "tank-remnants",
		position = {position.x + offset.x + 4, position.y + offset.y + 5},
		force = "neutral"
	}
